...
async function test_dailyDistributionEvent_atMaxHWM() {
    const initial_supply = 1_000_000_000n;
    const comptokens_minted = 6_767n;
    ...
    context = await run_test("dailyDistribution At Max HWM", context, instructions, [context.payer], false, async (context, result) => {
        ...
        const future_ubi_interest = 1_187_629n; // interest_distribution / total_supply * future_ubi_amount
        Assert.assertEqual(
...
(async () => { await test_dailyDistributionEvent_atMaxHWM(); })();
