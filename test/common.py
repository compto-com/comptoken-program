...
class PDA(dict[str, Any]):
    ...
    def __init__(self, programId: str, *seeds: str | int | Pubkey) -> None:
        ...
        super().__init__(
            json.loads(
                run(f"solana find-program-derived-address {programId} {seeds_str} --output json"
                    )))
    ...
    while True:
        try:
            ...
            raise SubprocessFailedException(
                f"Failed to run command! command: '{command}' timed out")
...
def generateComptokenAddressFile(globalDataSeed: int, interestBankSeed: int,
                                 UBIBankSeed: int, earlyAdopterBankSeed: int,
                                 mint_address: str,
...
def generateTransferHookAddressFile(comptoken_address: str,
                                    extraAccountMetasSeed: int,
                                    mint_address: str, interest_address: str,
                                    ubi_address: str):
