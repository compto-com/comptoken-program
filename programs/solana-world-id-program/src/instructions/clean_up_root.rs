...
impl<'info> CleanUpRoot<'info> {
    pub fn constraints(ctx: &Context<Self>) -> Result<()> {
        ...
        require!(root.root != latest_root.root, SolanaWorldIDProgramError::RootIsLatest);

        // Check that the root has expired.
        let current_timestamp = Clock::get()?.unix_timestamp.try_into().expect("timestamp underflow");
        require!(!root.is_active(&current_timestamp, &config.root_expiry), SolanaWorldIDProgramError::RootUnexpired);
