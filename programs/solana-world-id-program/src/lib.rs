...
declare_id!("5a3BkvmSEWSkWnBaFZGAUiywmjoqBqzspje9UmxcwG6L");
...
pub mod solana_world_id_program {
    ...
    pub fn post_signatures(
        ctx: Context<PostSignatures>, guardian_signatures: Vec<[u8; 66]>, total_signatures: u8,
    ) -> Result<()> {
        instructions::post_signatures(ctx, guardian_signatures, total_signatures)
    }

    pub fn update_root_with_query(
        ctx: Context<UpdateRootWithQuery>, bytes: Vec<u8>, root_hash: [u8; 32], guardian_set_index: u32,
    ) -> Result<()> {
