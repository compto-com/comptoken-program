...
async function test_dailyDistributionEvent_belowMaxHWM() {
    const initial_supply = 1_000_000_000n;
    const comptokens_minted = 6_760n;
    ...
    const high_watermark_increase = high_watermark_increase_uncapped < max_hwm_increase ? high_watermark_increase_uncapped : max_hwm_increase;
    ...
    context = await run_test("dailyDistribution Below Max HWM", context, instructions, [context.payer], false, async (context, result) => {
        ...
        const supply_increase = 1_460_000n; // high_watermark_increase (10) * COMPTOKEN_DISTRIBUTION_MULTIPLIER (146,000)
        Assert.assertEqual(
        ...
        const future_ubi_interest = 698_605n; // interest_distribution / total_supply * future_ubi_amount
        Assert.assertEqual(
...
(async () => { await test_dailyDistributionEvent_belowMaxHWM(); })();
