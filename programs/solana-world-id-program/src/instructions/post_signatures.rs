...
impl<'info> PostSignatures<'info> {
    pub fn constraints(guardian_signatures: &[[u8; 66]]) -> Result<()> {
        // Signatures should not be empty, since this is used by is_initialized.
        // Additionally, there is no reason for it to be.
        require!(!guardian_signatures.is_empty(), SolanaWorldIDProgramError::EmptyGuardianSignatures);
...
pub fn post_signatures(
    ctx: Context<PostSignatures>, mut guardian_signatures: Vec<[u8; 66]>, _total_signatures: u8,
) -> Result<()> {
