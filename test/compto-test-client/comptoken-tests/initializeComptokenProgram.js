import { GlobalDataAccount, TokenAccount } from "@compto/comptoken.js";
import { AccountState } from "@solana/spl-token";

import {
    ExtraAccountMetaAccount,
...
async function initialize_comptoken_program() {
    ...
    context = await run_test("initializeComptokenProgram", context, instructions, [context.payer], false, async (context, result) => {
        ...
        const final_interest_bank = await get_account(context, compto_public_keys.interest_bank_account_pubkey, TokenAccount);
        Assert.assertEqual(final_interest_bank.data.amount, 0n, "interest amount");
        Assert.assert(final_interest_bank.data.mint.equals(compto_public_keys.comptoken_mint_pubkey), "interest mint");
        Assert.assert(final_interest_bank.data.owner.equals(compto_public_keys.global_data_account_pubkey), "interest owner");
        Assert.assertEqual(final_interest_bank.data.state, AccountState.Initialized, "interest state");

        const final_UBI_bank = await get_account(context, compto_public_keys.verified_human_ubi_bank_account_pubkey, TokenAccount);
        Assert.assertEqual(final_UBI_bank.data.amount, 0n, "ubi amount");
        Assert.assert(final_UBI_bank.data.mint.equals(compto_public_keys.comptoken_mint_pubkey), "ubi mint");
        Assert.assert(final_UBI_bank.data.owner.equals(compto_public_keys.global_data_account_pubkey), "ubi owner");
        Assert.assertEqual(final_UBI_bank.data.state, AccountState.Initialized, "ubi state");

        const final_extra_account_metas_account = await get_account(context, compto_public_keys.compto_extra_account_metas_account_pubkey, ExtraAccountMetaAccount);
