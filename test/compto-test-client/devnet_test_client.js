...
async function createUserComptokenTokenAccount() {
    let createTokenAccountResult = await createAssociatedTokenAccount(connection, testUser_keypair, comptoken_mint_pubkey, testUser_keypair.publicKey, undefined, TOKEN_2022_PROGRAM_ID);
...
async function createGlobalDataAccount() {
    ...
    let createGlobalDataAccountResult = await sendAndConfirmTransaction(connection, createGlobalDataAccountTransaction, [testUser_keypair, testUser_keypair], { commitment: 'confirmed' });
