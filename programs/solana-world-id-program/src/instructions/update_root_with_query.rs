...
fn test_latest_root_signature() {
    let hash = keccak::hashv(&[b"latestRoot()"]).to_bytes();
...
pub struct UpdateRootWithQuery<'info> {
    ...
    #[account(
        ...
        bump,
        seeds::program = Pubkey::new_from_array(CORE_BRIDGE_PROGRAM_ID.to_bytes())
    )]
...
impl<'info> UpdateRootWithQuery<'info> {
    pub fn constraints(ctx: &Context<Self>, bytes: &[u8]) -> Result<()> {
        ...
        require!(guardian_set.is_active(&timestamp), SolanaWorldIDProgramError::GuardianSetExpired);

        // Compute the message hash.
        let message_hash = [MESSAGE_PREFIX, &keccak::hashv(&[bytes]).to_bytes()].concat();
        ...
        require!(guardian_signatures.len() >= quorum, SolanaWorldIDProgramError::NoQuorum);
        ...
        for sig_bytes in guardian_signatures {
            ...
            if let Some(last_index) = last_guardian_index {
                require!(index > last_index, SolanaWorldIDProgramError::InvalidGuardianIndexNonIncreasing);
            }
...
pub fn update_root_with_query(
    ...
    let response = QueryResponse::deserialize(&bytes).map_err(|_| SolanaWorldIDProgramError::FailedToParseResponse)?;
    // Requests can be batched, expect only one request to Ethereum.
    require!(response.request.requests.len() == 1, SolanaWorldIDProgramError::InvalidNumberOfRequests);
    let request = &response.request.requests[0];
    require!(request.chain_id == ETH_CHAIN_ID, SolanaWorldIDProgramError::InvalidRequestChainId);
    ...
    require!(query.call_data.len() == 1, SolanaWorldIDProgramError::InvalidRequestCallDataLength);
    require!(query.call_data[0].to == ETH_WORLD_ID_IDENTITY_MANAGER, SolanaWorldIDProgramError::InvalidRequestContract);
    require!(query.call_data[0].data == LATEST_ROOT_SIGNATURE, SolanaWorldIDProgramError::InvalidRequestSignature);
    // Sanity checks: expect one EthCall response from Ethereum.
    require!(response.responses.len() == 1, SolanaWorldIDProgramError::InvalidNumberOfResponses);
    let response = &response.responses[0];
    require!(response.chain_id == ETH_CHAIN_ID, SolanaWorldIDProgramError::InvalidResponseChainId);
    ...
    require!(result.as_slice() == root_hash, SolanaWorldIDProgramError::RootHashMismatch);
 ...
 */
fn verify_guardian_signature(sig: &GuardianSetSig, guardian_pubkey: &[u8; 20], digest: &[u8]) -> Result<()> {
    ...
    require!(recovered == *guardian_pubkey, SolanaWorldIDProgramError::InvalidGuardianKeyRecovery);
