...
export function hashToField(input: Uint8Array | string) {
    const bytes = typeof input === "string" && !input.startsWith("0x") ? Buffer.from(input) : input;
...
// But in TypeScript
export function appIdActionToExternalNullifierHash(appId: string, action: string) {
