...
async function test_unverifyHuman2Instruction() {
    // appId:  "self_hosted"
    ...
    const existing_accounts = [
        ...
        get_default_extra_account_metas_account(), world_id_config_account, world_id_latest_root_account, world_id_root_account, nullifier_account,
    ];
    ...
    const instructions = [
        ...
        await createUnverifyHuman2Instruction(
    ...
    context = await run_test("UnverifyHuman2", context, instructions, [context.payer, user], false, async (context, result) => {
        ...
        // unverifyHuman increases inactiveVerifiedHumans rather than decreasing verifiedHumans to prevent double dipping on "future" ubi
        Assert.assertEqual(
...
(async () => { await test_unverifyHuman2Instruction(); })();